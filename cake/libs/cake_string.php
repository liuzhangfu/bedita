 ...
 * Example: `CakeCakeString::insert(':name is :age years old.', array('name' => 'Bob', '65'));`
 ...
 * - clean: A boolean or array with instructions for CakeCakeString::cleanInsert
 *
 ...
 */
	function insert($str, $data, $options = array()) {
		...
		$data = (array)$data;
		if (empty($data)) {
			return ($options['clean']) ? CakeCakeString::cleanInsert($str, $options) : $str;
		}
...
/**
 * Cleans up a CakeCakeString::insert() formated string with given $options depending on the 'clean' key in
 * $options. The default method used is text but html is also available. The goal of this function
 * is to replace all whitespace and uneeded markup around placeholders that did not get replaced
 * by CakeCakeString::insert().
 *
 * @param string $str
		...
		switch ($clean['method']) {
			case 'html':
				...
				if ($clean['andText']) {
					...
					$str = CakeCakeString::cleanInsert($str, $options);
				}
				break;
			case 'text':
				$clean = array_merge(array(
